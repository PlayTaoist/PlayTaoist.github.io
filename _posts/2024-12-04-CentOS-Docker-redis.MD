...
Connot connect to the Docker daemon at unix:///var/run/docker.sock. Is the docker daemon running? 代表没有启动成功，需要查询原因
...
A. 修改redis.conf中的dir配置，改为如下/data工作目录

```shell
dir /data
```

B.logfile配置，修改log路径为/data目录下

```shell
logfile /data/redis.log
```

C.其他配置自行修改，比如 equirepass,bind,protected-mode等。
...
可以使用docker ps命令查看下，有一个任务正在执行 `docker ps`

以上步骤就可以链接redis了，并且数据文件已经持久化，会看到2个文件，一个是redis的log文件，一个是redis数据文件
...
3.运行save命令，将redis数据持久化，会出现dump.rdb文件。
